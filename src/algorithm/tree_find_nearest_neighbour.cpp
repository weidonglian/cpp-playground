...
const kd_node *kd_find_insert_position(const kd_node *root, point pt,
    ...
    const int dim = depth % k_dims;
    if (pt[dim] < root->pt[dim]) {
        return root->lo ? kd_find_insert_position(root->lo, pt, depth + 1)
                        : root;
    } else {
        return root->hi ? kd_find_insert_position(root->hi, pt, depth + 1)
                        : root;
    }
}
...
const kd_node *kd_find_nearest(const kd_node *root, point pt_query,
    ...
    if (should_search_lo && root->lo) {
        const kd_node *best_lo =
            kd_find_nearest(root->lo, pt_query, min_dist, depth + 1);
    ...
    if (should_search_hi && root->hi) {
        const kd_node *best_hi =
            kd_find_nearest(root->hi, pt_query, min_dist, depth + 1);
        if (best_hi) {
...
void dump_svg(const string &file_name, const vector<point> &points, point test,
              point q_linear, point q_kd_tree) {
    using namespace svg;
    ...
    for (const auto &p : points) {
        ...
        if (q_kd_tree.value() != q_linear.value()) {
            details::dump_svg("find_nearest_neighbour_random" +
                                  to_string(cnt++) + ".svg",
                              points, p, q_linear.value(), q_kd_tree.value());
        }
    }
}
...
TEST(TreeSuite, find_nns_query_not_necessarily_from_cluster) {
    ...
    const vector<point> points_to_test =
        details::generate_random_cloud_points(1000);
    int cnt = 0;
