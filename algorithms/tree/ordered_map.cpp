...
public:
  ...
  ~ordered_map() { bst_delete(head); }

  void insert(key_type key, value_type val) { head = bst_insert(head, move(key), move(val), pred); }

  void erase(const key_type& key) { head = bst_remove(head, key, pred); }
