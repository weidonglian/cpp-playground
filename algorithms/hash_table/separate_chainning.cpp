...
template <typename K, typename V, class Hash = hash<K>, class Pred = equal_to<K>>
class hash_table {
public:
  ...
  hash_table()
  : bucket_size_(k_def_bucket_size), buckets_(make_unique<hash_node*[]>(bucket_size_)), hasher_(), equal_pred_() {}
...
private:
  ...
  hash_node* hash_node_new(key_type key, value_type val) { return new hash_node{move(key), move(val), nullptr}; }
  ...
  [[nodiscard]] int bucket_index(const key_type& key) const { return hasher_(key) % bucket_size_; }
