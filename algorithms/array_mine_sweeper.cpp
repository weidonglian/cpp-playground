...
vector<vector<int>> generate_mine_sweeper_board(const vector<position> &bombs,
                                                int n, int m) {
        ...
        for (int i = -1; i <= 1; i++)
            for (int j = -1; j <= 1; j++) {
                if (i == 0 && j == 0) {
                    ...
                    if (is_valid(idx, n) && is_valid(idy, m) &&
                        board[idx][idy] != -1)
                        board[idx][idy] += 1;
                }
            }
...
void mine_sweeper_click_to_expand(vector<vector<int>> &field, int click_x,
    ...
    if (!is_valid(click_x, n) || !is_valid(click_y, m) ||
        field[click_x][click_y])
        return;
...
TEST_CASE("mine_sweeper_where_to_expand", "[array]") {
    ...
    const vector<vector<int>> field2 = {
