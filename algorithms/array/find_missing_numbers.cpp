...
// search the first element via divide-and-conquer.
...
// search the first element iteratively.
...
/**
 * @brief Find the missing elements between two given numbers from a sequence.
 *
 * 1. Write a simple binary search in divide-and-conquer and iterative styles.
 *    This will shrink the search range.
 * 2. Find the missing number and skip the duplicated ones.
 *
 */
TEST_CASE("find_missing_numbers", "[array]") {
  ...
  CHECK(find_missing_numbers({1, 3, 7, 9, 17, 23}, 3, 9) == std::vector<int>({4, 5, 6, 8}));
  CHECK(find_missing_numbers({1, 3, 3, 7, 9, 9, 17, 23}, 3, 9) == std::vector<int>({4, 5, 6, 8}));
  CHECK(find_missing_numbers({1, 3, 3, 7, 7, 9, 9, 17, 20}, 1, 20) ==
